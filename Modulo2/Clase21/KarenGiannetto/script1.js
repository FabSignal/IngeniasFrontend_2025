...
function sumaImportes (importes) {
    ...
    for (let i=0; i < importes.length; i++) {     
        if (importes[i] > 0 && importes[i] <= 1000) {      //si el contenido del vector en pos i cumple sumo al total 
            total += importes[i];
        }
    }
    return total;
...
console.log(`Total de montos del array mayor que no superan los 1000: ${resultado}`);
